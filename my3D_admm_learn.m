...
for i=1:max_it
    ...
    [zhatT_blocks, invzhatTzhat_blocks] = myprecompute_Z_hat_d(z_hat,size_z,gammas_D);
    for i_d=1:max_it_d
        %v_D是中间变量vD1是dz
...
% dhat_flat = reshape( dhat, ss, [] );%ss,k(10)
% size(dhat_flat)
...
ss = prod(size_z(1:ndim));%sx*sy*sz
rho=gammas(2)/gammas(1);
%Precompute spectra for H 然后就计算
zhatT_blocks = reshape( num2cell( permute( reshape(z_hat, [ss, k, n] ), [3,2,1] ), [1 2] ), [1 ss]); %n * k * s 先将前面的部分转换成元胞然后在reshape为1 ss的形式

%Precompute the inverse matrices for each frequency
invzhatTzhat_blocks = reshape( cellfun(@(A)(1/rho * eye(k) - 1/rho * A'*pinv(rho * eye(n) + A * A')*A), zhatT_blocks, 'UniformOutput', false'), [1 ss]); 
%reshape成1 ss的形式了 1x所有像素 的形式
